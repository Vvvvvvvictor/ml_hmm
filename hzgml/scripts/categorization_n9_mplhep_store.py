...
# base_dir = "/eos/home-j/jiehan/root_mumu/outputs_ggH_oreso_24_rough_bdt_1130/ggH"
base_dir = "/eos/home-j/jiehan/root_mumu/outputs_ggH_oreso_reweight_24_bdt_1203/ggH"
...
def compute_significance(sig_scores, sig_weights, sig_masses,
    ...
    return_per_bin: 如果为True，返回(总significance, per_bin_info列表)
                    per_bin_info包含每个bin的S, B, Z, 窗口信息
    """
    S_list = []
    B_list = []
    window_list = []  # 记录每个bin使用的质量窗口
    lower = -np.inf
    ...
    if return_per_bin:
        per_bin_info = []
        for i, (S, B, Z, window) in enumerate(zip(S_list, B_list, sig_per_cat, window_list)):
            per_bin_info.append({
                'bin_idx': i,
                'S': S,
                'B': B,
...
if __name__ == "__main__":
    ...
    print(f"\nGlobal fixed window used for smooth curve: [{g_lo:.3f}, {g_hi:.3f}] GeV "
