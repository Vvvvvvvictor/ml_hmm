#!/usr/bin/env python
"""Apply NN VBF models to ROOT files - adapted from apply_bdt.py"""
...
def getArgs():
    ...
    parser.add_argument('-c', '--config', action='store', nargs=2, default=['data/training_config_NN_VBF.json', 'data/apply_config_NN.json'], help='Config files: training and apply')
    ...
    parser.add_argument('-m', '--modelFolder', action='store', default='models_VBF_dnn', help='directory of NN models')
    parser.add_argument('-o', '--outputFolder', action='store', default='outputs', help='directory for outputs')
    parser.add_argument('-r', '--region', action='store', choices=['two_jet', 'one_jet', 'zero_jet', 'zero_to_one_jet', 'VH_ttH', 'all_jet', "ggH", "VBF", "VBFsep"], default='two_jet', help='Region to process')
    parser.add_argument('-cat', '--category', action='store', nargs='+', help='apply only for specific categories')
    parser.add_argument('--tree-name', action='store', default='two_jet_m110To150', help='Name of the tree in ROOT files')
...
class ApplyNNHandler(object):
    ...
    def loadModels(self):
        """Load Keras NN models."""
        if self.models:
            for model in self.models:
                print('NN INFO: Loading NN model: ', model)
                self.m_models[model] = []
                for i in range(4):
    ...
    def loadScaler(self):
        ...
        if self.models:
            for model in self.models:
                ...
                # Try to load DNN score transformers if present
                self.m_tsfs[model] = []
                for i in range(4):
                    tsf_path = '%s/DNN_tsf_%d.pkl' % (self._modelFolder, i)
                    if os.path.exists(tsf_path):
                        try:
                            tsf = pickle.load(open(tsf_path, 'rb'))
                            print(f'  Loaded transformer for fold {i}: {tsf_path}')
                        except Exception as e:
                            print(f'NN WARNING: Failed to load transformer {tsf_path}: {e}')
                            tsf = None
                    else:
                        tsf = None
    ...
    def applyNN(self, category, scale=1, shift=0):
        """Apply NN models to category samples."""
    ...
    def _process_files(self, f_list, output_path, branches, outputbraches, scale, shift, category):
        ...
        for file_idx, filename in enumerate(tqdm(sorted(f_list), desc='NN INFO: Applying NNs to %s samples' % category, bar_format='{desc}: {percentage:3.0f}%|{bar:20}{r_bar}')):
            try:
            ...
            for data in file[self._inputTree].iterate(library='pd', step_size=self._chunksize):
                ...
                for i in range(4):
                    ...
                    for model in self.train_variables.keys():
                        ...
                        # Predict using the NN model with two inputs
                        scores = self.m_models[model][i].predict([x_mass, x_topo], batch_size=81920, verbose=0)
                        scores = scores.flatten()  # Flatten to 1D array
                        
                        nn_basename = self.models[model]
                        data_o[nn_basename] = scores
                        # Try to apply a saved transformer (if available) to get _t version
                        tsf = None
                        if model in self.m_tsfs:
                            try:
                                tsf = self.m_tsfs[model][i]
                            except Exception:
                                tsf = None
                        if tsf is not None:
                            try:
                                scores_t = tsf.transform(scores.reshape(-1, 1)).reshape(-1)
                            except Exception as e:
                                print(f'NN WARNING: transformer failed to transform scores for model {model} fold {i}: {e}')
                                scores_t = scores
                        else:
                            scores_t = scores
...
def main():
    ...
    nn = ApplyNNHandler(configPath, args.region)
